...
class GetMoveHandler(webapp.RequestHandler):
  ...
  def post(self):
    ...
    self.response.headers['Cache-Control'] = 'no-cache, no-store, no-transform'
    ...
    if self.request.get('state'):
      ...
      game = State(self.request.get('state'))  # Load the state of the game
      last_move = game.moves[-1]
      from bot import Bot
      # Is it the bot's turn?
      if not game.outcome and last_move and last_move.player == 1 and last_move.type != COLLAPSE:
        #if self.debug(game): return
        # Send move notation to player
        self.response.out.write(Bot.play_move(game, int(self.request.get('difficulty'))))
